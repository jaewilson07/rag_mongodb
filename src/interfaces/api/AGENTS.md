...
FastAPI application with REST API endpoints for ingestion, querying, wiki generation, and the save-and-research reading pipeline. All business logic lives in `services/`; routes are thin wrappers. Canonical entry: `mdrag.interfaces.api.main:app`. `server/` re-exports for backward compatibility.
...
| `dependencies.py` | `ManagedDependencies`, `get_agent_dependencies()` |
| `api/*/router.py` | Route handlers (feedback, health, ingest, query, readings, wiki) |
| `services/*.py` | `WikiService`, `ReadingsService`, `QueryAPIService`, `IngestJobService` |
| `config/api_config.py` | `APIConfig` |
...
```
rg "class.*Service" src/interfaces/api/services
rg "include_router" src/interfaces/api/main.py
```
