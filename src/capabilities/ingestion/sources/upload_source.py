...
def _to_namespace(ns: object) -> Namespace:
    """Convert namespace-like object to Namespace (handles cross-module type mismatch)."""
    if hasattr(ns, "model_dump"):
        return Namespace(**ns.model_dump())
    return Namespace(
        user_id=getattr(ns, "user_id", None),
        org_id=getattr(ns, "org_id", None),
        source_group=getattr(ns, "source_group", None),
    )
    ...
    async def collect(self, request: UploadCollectionRequest) -> List[CollectedSource]:
        ...
        if request.file_path:
            ...
            collected = CollectedSource(
                ...
                namespace=_to_namespace(request.namespace),
            )
            return [collected]
